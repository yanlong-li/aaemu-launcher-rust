use crate::win_main::WmCommand::Exit;
use crate::win_main::{SafeHWND, EXIT_GAME_BUTTON_HWND, HBITMAP_BG};
...
pub fn create(hwndparent: HWND) {
    unsafe {
        EXIT_GAME_BUTTON_HWND.get_or_init(|| {
            let button = CreateWindowExW(
                ...
                30,
                hwndparent,
                HMENU(Exit as usize as *mut std::ffi::c_void),
...
pub fn brush(hwnd: HWND) {
    unsafe {
        let bg = HBITMAP(
            LoadImageW(
                GetModuleHandleW(None).unwrap(),
                PCWSTR(w!(r"./resources/exit.bmp").as_ptr()),
                IMAGE_BITMAP,
                0,
                0,
                LR_LOADFROMFILE,
